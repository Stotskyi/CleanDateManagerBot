...
public class UserService(IUserRepository userRepository)
{
    ...
    public async Task<string> HandleDickCommandAsync(string username,string firstname,string lastname)
    {
        ...
        return $"{user.Username} твій пісюн {(user.DickSize > 0 ? "виріс" : "зменшився")} на {Math.Abs(randomNumber)} см . Тепер його довжина: {user.DickSize} см . Продовжуй грати завтра";
    }
